...
test.describe('Language Switching', () => {
  ...
  test('should default to Japanese', async ({ page }) => {
    ...
    await expect(page.locator('button.tab:has-text("結果")')).toBeVisible();
    ...
    // Japanese radio should be checked
  ...
  test('should switch to Chinese (zh-TW)', async ({ page }) => {
    ...
    await expect(page.locator('button.tab:has-text("結果")')).toBeVisible();  // Same
  ...
  test('should update management section text in Chinese', async ({ page }) => {
    // Switch to Chinese
    const zhRadio = page.locator('input[type="radio"][value="zh-TW"]');
    await zhRadio.click();
    await page.waitForTimeout(500);

    // Verify specific management text changed
    await expect(page.locator('h2', { hasText: '表格管理' })).toBeVisible();   // テーブル管理 -> 表格管理
    await expect(page.locator('h2', { hasText: '編輯模式' })).toBeVisible();   // 編集モード -> 編輯模式
    ...
    // Button/badge text should change
    await expect(page.locator('text=使用中')).toBeVisible();         // Same: 使用中
  });
  ...
  test('should update table header when language changes', async ({ page }) => {
    ...
    await page.click('button.tab:has-text("結果")');
    await page.waitForTimeout(200);
  ...
  test('should update help page text when language changes', async ({ page }) => {
    ...
    // Go back to management and switch language
